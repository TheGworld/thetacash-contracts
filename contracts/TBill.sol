...
import "./interfaces/IRebasingTokenXC.sol";
import "./lib/SafeMathInt.sol";

/**
 * @title TBILL ERC20 token
 * @dev This is derivate of part of an implementation of the TBill Ideal Money protocol.
 *      TBill is a normal ERC20 token, but its supply can be adjusted by splitting and
 *      combining tokens proportionally across all wallets.
 *
 *      TBILL balances are internally represented with a hidden denomination, 'gons'.
 *      We support splitting the currency in expansion and combining the currency on contraction by
 *      changing the exchange rate between the hidden 'gons' and the public 'fragments'.
 */
contract TBill is ERC20Detailed, Ownable, ITokenMetaData, IRebasingTokenXC {
    ...
    event LogTokenUriUpdated(string tokenURI);    
    event LogMint(address toAddress, uint256 value);
    event LogMintDisabled();
    event LogBurn(address fromAddress, uint256 value);


    modifier onlyMonetaryPolicy() {
        require(msg.sender == monetaryPolicy, "can only be called by monetary policy contract");
        _;
    }
    ...
    // EIP-1046: URI of JSON describing a token,
    // including an image URI
    // (see https://eips.ethereum.org/EIPS/eip-1046#specification)
    ...
    function initialize(address owner_) public override initializer {
        ERC20Detailed.initialize("ThetaCash", "TBILL", uint8(DECIMALS));
        Ownable.initialize(owner_);

        _mintDisabled = false;
        _tokenURI = "ipfs://Qmabz7EQX1vV8aVZxWUNZBUVzSRcKr4gdytXKJqUg3Vzab";
        _totalGons = MAX_UINT128 - (MAX_UINT128 % INITIAL_FRAGMENTS_SUPPLY);
    ...
    function setTokenURI(string memory tokenURI) external onlyOwner {
        _tokenURI = tokenURI;
        emit LogTokenUriUpdated(tokenURI);
    }
    ...
    */
    ...
    function mint(address to, uint256 value) external onlyOwner validRecipient(to) {
        require(!_mintDisabled, "mint disabled");
        ...
        emit LogMint(to, value);
    }

    function mintMulti(address[] calldata toList, uint256[] calldata valueList) external onlyOwner {
        require(!_mintDisabled, "mint disabled");
    ...
    function transferMulti(address[] calldata toList, uint256[] calldata valueList) external returns (bool) {
        ...
        for (uint i = 0; i < toList.length; ++i){
            ...
            require(to != address(this), "cannot be own address");
            value = valueList[i];
            gonValue = value.mul(_gonsPerFragment);
            gonSum = gonSum.add(gonValue);
            _gonBalances[to] = _gonBalances[to].add(gonValue);
            emit Transfer(msg.sender, to, value);
        }
    ...
    function permit(
        ...
        require(owner == ecrecover(digest, v, r, s), "not owner");
